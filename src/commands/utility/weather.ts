...
export default new Command({
  ...
  run: async({ interaction }) => { //, client }) => {
    ...
    const coordinates: Coordinates = ((await (await fetch(process.env.ADDRESS_API + encodeURIComponent(location))).json()) as { results: Coordinates[] }).results[0];
    ...
    const localTime = new Date((data.current.dt+data.timezone_offset - 43200) * 1000);

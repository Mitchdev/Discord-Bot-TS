...
export default new Command({
  ...
  options: [{
    ...
    description: 'Get the trailer of a film',
    options: [{
      name: 'search',
    ...
    name: 'plot',
    type: ApplicationCommandOptionType.Subcommand,
    description: 'Get the plot of a film',
    options: [{
      name: 'search',
      type: ApplicationCommandOptionType.String,
      description: 'Search for film',
    ...
    name: 'stats',
    type: ApplicationCommandOptionType.Subcommand,
    description: 'Get the stats of a film',
    options: [{
      name: 'search',
      type: ApplicationCommandOptionType.String,
      description: 'Search for film',
    ...
    if (searchResult.errorMessage === '') {
      if (searchResult.results.length > 0) {
        if (subCommand === 'trailer') {
          const trailerData = await (await fetch(process.env.IMDB_TRAILER_API.replace('|id|', searchResult.results[0].id), {redirect: 'follow'})).json() as ImdbTrailer;
          if (trailerData.videoUrl) {
            interaction.editReply({content: `**${trailerData.fullTitle ?? trailerData.title}**\n${trailerData.videoUrl}`});
          } else {
            interaction.editReply({content: 'Could not find a trailer'});
          }
        } else {
